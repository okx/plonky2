...
impl ElementBN128 {
    #[inline]
    fn mul64(self, a: u64, b: u64) -> (u64, u64) {
        let c128: u128 = (a as u128) * (b as u128);
        ((c128 >> 64) as u64, c128 as u64)
    }
    ...
    fn _mulGeneric(self, x: [u64; 4], y: [u64; 4]) -> [u64; 4] {
        ...
        (c[1], c[0]) = self.madd1(v, y[2], c[1]);
        (c[2], t[1]) = self.madd2(m, 13281191951274694749u64, c[2], c[0]);
        (c[1], c[0]) = self.madd1(v, y[3], c[1]);
        (t[3], t[2]) = self.madd3(m, 3486998266802970665u64, c[0], c[2], c[1]);
        ...
        (c[2], t[0]) = self.madd2(m, 2896914383306846353u64, c[2], c[0]);
        (c[1], c[0]) = self.madd2(v, y[2], c[1], t[2]);
        (c[2], t[1]) = self.madd2(m, 13281191951274694749u64, c[2], c[0]);
        (c[1], c[0]) = self.madd2(v, y[3], c[1], t[3]);
        (t[3], t[2]) = self.madd3(m, 3486998266802970665u64, c[0], c[2], c[1]);
        ...
        (c[1], c[0]) = self.madd2(v, y[2], c[1], t[2]);
        (c[2], z[1]) = self.madd2(m, 13281191951274694749u64, c[2], c[0]);
        (c[1], c[0]) = self.madd2(v, y[3], c[1], t[3]);
        (z[3], z[2]) = self.madd3(m, 3486998266802970665u64, c[0], c[2], c[1]);

        // if z > q --> z -= q
        // note: this is NOT constant time
        if !(z[3] < 3486998266802970665u64
            || (z[3] == 3486998266802970665u64
                && (z[2] < 13281191951274694749u64
                    || (z[2] == 13281191951274694749u64
                        && (z[1] < 2896914383306846353u64
                            || (z[1] == 2896914383306846353u64
    ...
    fn _fromMontGeneric(self, x: [u64; 4]) -> [u64; 4] {
        ...
        (c, z[2]) = self.madd2(m, 3486998266802970665u64, z[3], c);
        z[3] = c;
...
#[derive(Copy, Clone, Default, Debug, PartialEq)]
pub struct PoseidonBN128NativePermutation<F> {
