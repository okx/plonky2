...
use super::poseidon::{PoseidonHash, PoseidonPermutation};
...
mod tests {
    ...
    fn test_hash12() {
        ...
        let expected_result: [[u64; 4]; 9] = [
            ...
            [
                7986352640330579808,
                16698976638447200418,
                14099060853601989680,
                1806029100513259151,
            ],
            [
                4912038160490892692,
                4103686885524875147,
                10541378107520313959,
                17279065624757782690,
            ],
            [
                7890399244011379224,
                3893786354640587971,
                13560196409483468805,
                2948784903943663078,
            ],
            [
                14700294369684741534,
                14895735373969203815,
                16434826207003907392,
                17867437290801947189,
            ],
            [
                17625602373883346164,
                18149609703473926001,
                2824810185344003270,
                9975387089464755098,
            ],
            [
                2691796281516679790,
                14584425213549820217,
                14318483276994184927,
                17940735015359233298,
            ],
            [
                8150678766998536923,
                14721645535435562842,
                2012097115710913290,
                6143064933387483688,
            ],
            [
    ...
    fn test_bench_hash12() {
        ...
        let mut input = [F::ZERO; 12];
        for i in 0..8 {
            input[i] = F::from_canonical_u64(inputs[i]);
        }
        let mut output: [F; 4] = [F::ZERO; 4];

        const REPS: usize = 1 << 20;

        let t0 = Instant::now();
        for _i in 0..REPS {
            output = PoseidonHash::hash12(&input);
        }
        let tdiff = Instant::now().duration_since(t0).as_nanos();

        assert_eq!(output[0].to_canonical_u64(), 7986352640330579808);
        assert_eq!(output[1].to_canonical_u64(), 16698976638447200418);
        assert_eq!(output[2].to_canonical_u64(), 14099060853601989680);
        assert_eq!(output[3].to_canonical_u64(), 1806029100513259151);
