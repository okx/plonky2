...
pub fn keccak_flex(inp: &[u8], inlen: usize, md: &mut [u8], mdlen: usize) {
    ...
    rsizw = rsiz / 8;
    st.fill(0);
    for _ in inp.chunks_exact(rsiz) {
        for i in 0..rsizw {
            st[i] ^= u64::from_ne_bytes(inp[i * 8..(i + 1) * 8].try_into().unwrap());
        }
        keccakf(&mut st, KECCAK_ROUNDS);
    }
    // last block and padding
    temp[..inlen].copy_from_slice(&inp);
    temp[inlen] = 1;
    temp[inlen + 1..rsiz].fill(0);
    temp[rsiz - 1] |= 0x80;
    for i in 0..rsizw {
